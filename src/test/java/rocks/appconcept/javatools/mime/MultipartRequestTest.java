...
public class MultipartRequestTest extends TestCase {
  ...
  public void testBetaSticker1Png() throws Exception {
    ...
    assertEquals("6abbcffd12b4ada5a6a084fe9e4584f846331bc4",
        sha1(req.getPart("sticker").getBinaryValue()));
  ...
  public void testSampleUpload() throws Exception {
    ...
    byte[] body = FileUtils
        .readFully(getClass().getClassLoader().getResourceAsStream("sample_upload.bin"));
    MultipartRequest req = MultipartRequest.parse(contentType, body);

    assertEquals(5, req.getParts().size());
    assertEquals("c0394cd18def09bf24031b8bae414260dc6c9772",
        sha1(req.getPart("file").getBinaryValue()));
