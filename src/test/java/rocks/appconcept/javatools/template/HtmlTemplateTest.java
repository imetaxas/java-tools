...
public class HtmlTemplateTest extends TestCase {
  ...
  public void testSimple() throws Exception {
    HtmlTemplate template = new HtmlTemplate("hello $world! Variable: $variable",
        Collections.singletonMap("world", "COOL!"));
  ...
  public void testPresetConditionals() throws Exception {
    ...
    template = new HtmlTemplate("#if($a)a conditional#end",
        Collections.singletonMap("a", (String) null));
    writer = new ByteArrayOutputStream();
    template.renderTemplate(Collections.singletonMap("a", "1"), writer);
    assertEquals("conditional should be precompiled away", "",
        new String(writer.toByteArray(), UTF_8));

    template = new HtmlTemplate("#if($a)a conditional#end", Collections.singletonMap("a", "1"));
    writer = new ByteArrayOutputStream();
    template.renderTemplate(null, writer);
    assertEquals("conditional should be precompiled on", "a conditional",
        new String(writer.toByteArray(), UTF_8));
  }
  ...
  public void testNonGreedyComments() throws Exception {
    ...
    new HtmlTemplate("#* this is a comment *#this is some text#* another comment *#", null)
        .renderTemplate(null, stream);
