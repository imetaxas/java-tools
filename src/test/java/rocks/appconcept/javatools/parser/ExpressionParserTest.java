...
public class ExpressionParserTest extends TestCase {
  ...
  public PegParser makeExpressionParser() {
    ...
    PegParser integer = PegParser.digits().make(Primary.class, "value");
    PegParser variable = PegParser.repeatPlus(PegParser.character(Character::isLetter))
        .make(Primary.class).set("value", "0");
    PegParser lookup = PegParser
        .sequence(variable, PegParser.literal("["), new AstQuotedString('"'),
            PegParser.literal("]")).make(Primary.class).set("value", "666");
    PegParser primary = PegParser.options(integer, lookup, variable,
        PegParser.sequence(PegParser.literal("("), temp2 = PegParser.call(), PegParser.literal(")"))
            .pick(1));
    PegParser multiplicative = PegParser.options(
        PegParser.sequence(primary, PegParser.literal("*"), temp = PegParser.call())
            .make(Multiplication.class, "left", null, "right"), primary);
    temp.resolve(multiplicative);
    PegParser additive = PegParser.options(
        PegParser.sequence(multiplicative, PegParser.literal("+"), temp = PegParser.call())
  ...
  public void testExpressionParser() throws Exception {
    ...
    assertEquals(15378,
        ((Node) parser.parse(PegParser.input("test+12+23*window[\"test\"]+(12*3+12)")).object)
            .getValue());
    ...
    assertEquals(10000 + 12 * (13 + 14),
        ((Node) parser.parse(PegParser.input("10000+12*(13+14)")).object).getValue());
  }
}
