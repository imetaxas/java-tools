...
public class ExpressionParserTest extends TestCase {
    ...
    public PegParser makeExpressionParser() {
        ...
        PegParser variable = PegParser.repeatPlus(PegParser.character(Character::isLetter)).make(Primary.class).set("value", "0");
        PegParser lookup = PegParser.sequence(variable, PegParser.literal("["), new AstQuotedString('"'), PegParser.literal("]")).make(Primary.class).set("value", "666");
        PegParser primary = PegParser.options(integer, lookup, variable, PegParser.sequence(PegParser.literal("("), temp2 = PegParser.call(), PegParser.literal(")")).pick(1));
        PegParser multiplicative = PegParser.options(PegParser.sequence(primary, PegParser.literal("*"), temp = PegParser.call()).make(Multiplication.class, "left", null, "right"), primary);
        temp.resolve(multiplicative);
        PegParser additive = PegParser.options(PegParser.sequence(multiplicative, PegParser.literal("+"), temp = PegParser.call()).make(Addition.class, "left", null, "right"), multiplicative);
