...
public class SamlResponse {
    ...
    public static SamlResponse parse(String samlResponse, String certificate, String privateKey) {
        try {
            // parse document
            DomWrapper root = DomWrapper.parseNS(new ByteArrayInputStream(parseBase64EncodedResponse(samlResponse)));
            ...
            String status = root.getOnlyNamedChild(SAMLP, "Status").getOnlyNamedChild(SAMLP, "StatusCode").getAttributeValue("Value");
            ...
            for (DomWrapper attribute : attributeStatement.getChildren(SAML, "Attribute")) {
                ...
                attributes.put(attributeName, attribute.getChildren(SAML, "AttributeValue").stream().map(DomWrapper::getElementText).collect(Collectors.toList()));
            }
    ...
    private static SamlResponse success(String issuer, String nameid, Map<String, List<String>> attributes) {
