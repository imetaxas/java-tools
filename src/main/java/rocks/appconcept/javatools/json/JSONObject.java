...
public abstract class JSONObject {
     ...
     */
    public static JSONObject parse(String json) { return new JSONObjectParser().read(json); }

    public static JSONObject object() { return new JSONObjectObject(new HashMap<String, JSONObject>());}
    public static JSONObject list() { return new JSONObjectList(new ArrayList<JSONObject>());}
    public static JSONObject bool(boolean val) { return val ? TRUE : FALSE; }
    public static JSONObject number(Number val) { return new JSONObjectNumber(val); }
    public static JSONObject string(String val) { return new JSONObjectString(val); }
     ...
     */
    public JSONObject get(int index) { throw new IllegalStateException("Cannot get index " + index + " of " + asString() + "."); }

    public void set(int index, JSONObject value) { throw new IllegalStateException("Cannot set index " + index + " of " + asString() + "."); }
     ...
     * @return The value of the property.
     */
    public JSONObject get(String property) { throw new IllegalStateException("Cannot get property " + property + " of " + asString() + "."); }

    public void set(String property, JSONObject value) { throw new IllegalStateException("Cannot set property " + property + " of " + asString() + "."); }
    ...
    public final String toString() { return toJSON(); }
     ...
     *
     * @return a set view of the mappings contained in this object, or an empty set if this is not an object.
     */
    public Set<Map.Entry<String, JSONObject>> entrySet() { return Collections.emptySet(); }

    /**
     * If this is an object or a list, returns a {@link Collection} view of the values contained in this object.
     * The collection is backed by the object or list, so changes to that are
     * reflected in the collection, and vice-versa.  If the object or list is
     * modified while an iteration over the collection is in progress
     * (except through the iterator's own <tt>remove</tt> operation),
     * the results of the iteration are undefined.  The collection
     * supports element removal, which removes the corresponding
     * value from the object or list, shifting any subsequent elements to the left in the case of a list, via the <tt>Iterator.remove</tt>,
     * <tt>Collection.remove</tt>, <tt>removeAll</tt>,
     * <tt>retainAll</tt> and <tt>clear</tt> operations.  It does not
     * support the <tt>add</tt> or <tt>addAll</tt> operations for objects, but does for lists.
     *
     * @return a collection view of the values contained in this object or list, or an empty collection if this is not an object or a list.
     */
    public Collection<JSONObject> values() { return Collections.emptySet(); }
    ...
    private static class JSONObjectParser {
        ...
        private JSONObject value(CharacterIterator json) {
            ...
            if (c == '"') {
                ...
                if (c(json) == 'r' && c(json) == 'u' && c(json) == 'e') return JSONObject.TRUE;
            } else if (c == 'f') {
                if (c(json) == 'a' && c(json) == 'l' && c(json) == 's' && c(json) == 'e') return JSONObject.FALSE;
            } else if (c == 'n') {
                if (c(json) == 'u' && c(json) == 'l' && c(json) == 'l') return JSONObject.NULL;
            }
            exception(json);
            return null;
        }
        ...
        private String string(CharacterIterator json) {
            ...
            while ((c = c(json)) != '"') {
                if (c == '\\') {
                    ...
                    else if (c == 'u') s.append(unicode(json));
                    else exception(json);
        ...
        private Number number(CharacterIterator json) {
            ...
            if (s.length() >= 3 && s.charAt(0) == '-' && s.charAt(1) == '0' && Character.isDigit(s.charAt(2)) || s.length() >= 2 && s.charAt(0) == '0' && Character.isDigit(s.charAt(1))) {
                exception(json);
            }
        ...
        private Object exception(CharacterIterator json) {
            ...
            for (int i = 0; i < 20; ++i) {
                if (json.getIndex() == index) s.append("!");
                s.append(json.current());
                if (json.next() == CharacterIterator.DONE) break;
            }
            throw new IllegalArgumentException("Malformed JSON near index " + index + ". (" + s.toString() + ")");
        }
