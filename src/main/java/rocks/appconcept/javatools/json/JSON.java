...
public class JSON {
    ...
    private static Object parse(Type type, JSONObject json) throws IllegalAccessException, InstantiationException {
            ...
            if (classType == Boolean.TYPE || classType == Boolean.class) {
                ...
                for (Field field : classType.getDeclaredFields()) {
                    if ((field.getModifiers() & Modifier.STATIC) == 0) {
                        ...
                        if (fieldValue != null || !field.getType().isPrimitive()) { // don't set null to primitive fields
    ...
    private static void stringify(JSONStream stream, String property, Object bean) throws InvocationTargetException, IllegalAccessException {
        if (bean == null) {
            ...
            if (valueClass.isArray()) {
                ...
                Optional<Method> toJSON = Stream.of(valueClass.getMethods()).filter(x -> x.getName().equals("toJSON") && x.getReturnType() == String.class && x.getParameterCount() == 0).findAny();
                if (toJSON.isPresent()) {
