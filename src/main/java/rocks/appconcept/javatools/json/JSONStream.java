...
/**
 * Utility class for writing JSON documents.
 * Example:
 *
 *     JSONStream json = new JSONStream();   // CURRENT STREAM:
 *     json.object();                        // {
 *     json.value("prop1", "a string");      // {"prop":"a string"
 *     json.value("prop2", 12.4);            // {"prop":"a string","prop2":12.4
 *     json.value("prop3", false);           // {"prop":"a string","prop2":12.4,"prop3":false
 *     json.list("listprop");                // {"prop":"a string","prop2":12.4,"prop3":false,"listprop":[
 *     json.value("another string");         // {"prop":"a string","prop2":12.4,"prop3":false,"listprop":["another string"
 *     json.value(12.5);                     // {"prop":"a string","prop2":12.4,"prop3":false,"listprop":["another string",12.5
 *     json.value(true);                     // {"prop":"a string","prop2":12.4,"prop3":false,"listprop":["another string",12.5,true
 *     json.endlist();                       // {"prop":"a string","prop2":12.4,"prop3":false,"listprop":["another string",12.5,true]
 *     json.endobject();                     // {"prop":"a string","prop2":12.4,"prop3":false,"listprop":["another string",12.5,true]}
...
public final class JSONStream {
    ...
    static {
        ...
        for (int i = 0; i < 32; ++i) table[i] = -(i + 1);
        table['"'] = '"';
        table['/'] = '/';
        table['\\'] = '\\';
        table[0x08] = 'b';
    ...
    public JSONStream object() {
        if (isObject()) throw new JSONException("Properties in objects must have a name");
        return object(null);
    }
    ...
    public JSONStream object(String prop) {
        if (isComplete()) throw new JSONException("No more objects can be created at this point");
        if (prop!=null && !isObject()) throw new JSONException("Property without object");
        if (isNotFirst()) out.append(",");
        if (prop != null) {
            string(prop);
            out.append(":");
        }
        out.append("{");
        if (stackptr>=0) stack[stackptr] |= NOTFIRST;
    ...
    public JSONStream verbatim(String prop, String json) {
        if (isComplete()) throw new JSONException("No more verbatim data can be output at this point");
        if (prop!=null && !isObject()) throw new JSONException("Property without object");
        if (isNotFirst()) out.append(",");
        if (prop != null) {
            string(prop);
    ...
    public JSONStream list(String prop) {
        if (isComplete()) throw new JSONException("No more list can be created at this point");
        if (prop!=null && !isObject()) throw new JSONException("Property without object");
        if (isNotFirst()) out.append(",");
        if (prop != null) {
            string(prop);
     ...
     *
     *  - This method can only be called if the current context is OBJECT or LIST.
     *  - After this method returns the current context is restored to what it was before the object or list was opened.
     *
     * @return The "this" JSONStream object, for easy chaining of calls.
     * @throws JSONException if the current context is EMPTY.
     */
    public JSONStream end() {
        if (stackptr <= 0) throw new JSONException("Close without open");
        out.append(isObject() ? "}" : "]");
        stackptr--;
        return this;
    }
    ...
    /**
     * Repeatedly write the end of a list or object depending on the current context until the document is valid.
     *
     *  - This method can only be called if the current context is OBJECT or LIST.
     *  - After this method returns the current context is EMPTY and no more writes should be made.
     *
     * @return The "this" JSONStream object, for easy chaining of calls.
     */
    public JSONStream endall() {
        while (stackptr > 0) end();
        return this;
    }
     ...
     *  - After this method returns the current context is restored to what it was before the list was opened.
     *
     * @return The "this" JSONStream object, for easy chaining of calls.
     * @throws JSONException if the current context is not LIST.
     */
    public JSONStream endlist() {
        if (stackptr <= 0) throw new JSONException("Close list without open");
        out.append("]");
        stackptr--;
        return this;
    }
     ...
     *
     *  - This method can only be called if the current context is OBJECT.
     *  - After this method returns the current context is restored to what it was before the object was opened.
     *
     * @return The "this" JSONStream object, for easy chaining of calls.
     * @throws JSONException if the current context is not OBJECT.
     */
    public JSONStream endobject() {
        if (stackptr <= 0) throw new JSONException("Close object without open");
        out.append("}");
        stackptr--;
        return this;
    }
    ...
    /**
     * Write a string value in an object context (also properly escapes the string as JSON): **"prop":STRING**
     *
     *  - This method can only be called if the current context is OBJECT.
     *
     * @param prop The name of the property (must not be ##null##).
    ...
    public JSONStream value(String prop, String str) {
        if (isComplete()) throw new JSONException("No more values can be created at this point");
        if (prop!=null && !isObject()) throw new JSONException("Property without object");
        if (isNotFirst()) out.append(",");
        if (prop != null) {
            string(prop);
    ...
    public JSONStream value(double obj) {
        if (isObject()) throw new JSONException("Properties in objects must have a name");
        if (Double.isInfinite(obj) || Double.isNaN(obj)) return value(null, Double.toString(obj));
        return value(null, obj);
    }
    ...
    public JSONStream value(String prop, double obj) {
        if (prop!=null && !isObject()) throw new JSONException("Property without object");
        if (Double.isInfinite(obj) || Double.isNaN(obj)) return value(prop, Double.toString(obj));
        if (isNotFirst()) out.append(",");
        if (prop != null) {
            string(prop);
            out.append(":");
    ...
    public String toString() {
        if (stackptr > 0) throw new JSONException("Unclosed json");
        return out.toString();
    }
