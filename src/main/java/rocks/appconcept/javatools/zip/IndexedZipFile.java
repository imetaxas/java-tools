...
/**
 * Indexed zip file reader.
 * <p>Returns input streams for archived files in constant(-ish) time.</p>
 * <p><b>Note:</b> Does not do any CRC32 verification.</p>
 * <p>Tries to handle filename encoding according to the "spec": http://www.pkware.com/documents/casestudies/APPNOTE.TXT</p>
 *
 * @author yanimetaxas
 */
public class IndexedZipFile {
    ...
    /**
     * Updates the specified digest with vital information on the contents of the file, while excluding some files.
     * @param signature The signature to update.
     * @param excluded The filenames to not include in the signature update. Can be null to include all content.
     */
    ...
    public InputStream getInputStream(String name) {
        ...
        if (index.containsKey(name)) {
            ...
            if (entry.streamLength == 0) return EmptyInputStream.instance;
            if (entry.compressionMethod == 0) {
                ...
                res = new InflaterInputStream(new ZeroPaddedByteArrayInputStream(file, entry.offset, entry.streamLength), new Inflater(true));
            } else {
    ...
    private static class EmptyInputStream extends InputStream {
        ...
        @Override public long skip(long n) throws IOException { return 0; }
    ...
    private void indexFileInternal(int startofCDR) {
        ...
        for (; ;) {
            ...
            if (left < 42) throw new IllegalArgumentException("Bad ZIP file. Premature end.");
            ...
            if (left < fnamelen) throw new IllegalArgumentException("Bad ZIP file. Premature end.");

            String name;
            // BIT 11 -> UTF-8 else IBM Codepage 437  (see http://www.pkware.com/documents/casestudies/APPNOTE.TXT )
            if ((flags & (1<<11)) == 0) {
                char [] namec = new char[fnamelen];
                for (int i=0; i<fnamelen; i++) namec[i] = cp437chars[file[p+i]&0xff];
                name = new String(namec);
            } else {
                name = getUTF8String(file, p, fnamelen);
            }

            p+=fnamelen; left-=fnamelen;
            if (left < extralen) throw new IllegalArgumentException("Bad ZIP file. Premature end.");
            p+=extralen; left-=extralen;
            if (left < commentlen) throw new IllegalArgumentException("Bad ZIP file. Premature end.");
            p+=commentlen; left-=commentlen;
    ...
    private static long getDword(byte[] file, int p) {
        return ((file[p]&0xff) | ((file[p+1]&0xff) << 8) | ((file[p+2]&0xff) << 16) | ((file[p+3]&0xff) << 24));
    }
