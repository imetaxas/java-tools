    ...
    public static class Part {
        ...
        private Part(byte[] data, int offset, int end) {
            ...
            while (data[cursor] != 0x0d || data[cursor + 1] != 0x0a) {
                ...
                String headerName = new String(data, headerNameStart, (headerNameEnd - headerNameStart), US_ASCII);

                cursor += 1; // skip ":"
                if (data[cursor] == ' ') cursor += 1; // optionally skip " "

                int headerValueStart = cursor;
                while (data[cursor] != 0x0d) cursor += 1;
                int headerValueEnd = cursor;
                String headerValue = new String(data, headerValueStart, (headerValueEnd - headerValueStart), US_ASCII);
