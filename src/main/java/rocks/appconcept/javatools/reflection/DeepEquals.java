...
public class DeepEquals {
   ...
   * @return true if a is equivalent to b, false otherwise.  Equivalent means that all field values
   * of both subgraphs are the same, either at the field level or via the respectively encountered
   * overridden .equals() methods during traversal.
   */
  public static boolean deepEquals(Object a, Object b) {
    ...
    while (!stack.isEmpty()) {
      ...
      if (dualKey._key1 == null || dualKey._key2
      ...
      if (!isContainerType(dualKey._key1) && !isContainerType(dualKey._key2) && !dualKey._key1
          .getClass().equals(dualKey._key2.getClass())) {   // Must be same class
        return false;
      }

      if (dualKey._key1 instanceof Double && compareFloatingPointNumbers(dualKey._key1,
          dualKey._key2, doubleEplison)) {
        continue;
      }

      if (dualKey._key1 instanceof Float && compareFloatingPointNumbers(dualKey._key1,
          dualKey._key2, floatEplison)) {
        continue;
      }
   ...
   * @param stack add items to compare to the Stack (Stack versus recursion)
   * @param visited Set of objects already compared (prevents cycles) value of 'true' indicates that
   * the Collections may be equal, and the sets items will be added to the Stack for further
   * comparison.
   */
  private static boolean compareOrderedCollection(Collection col1, Collection col2, Deque stack,
      Set visited) {
    // Same instance check already performed...
   ...
   * @return boolean false if the Collections are for certain not equals. A value of 'true'
   * indicates that the Collections may be equal, and the sets items will be added to the Stack for
   * further comparison.
   */
  private static boolean compareUnorderedCollection(Collection col1, Collection col2, Deque stack,
      Set visited) {
    // Same instance check already performed...
   ...
   * the maps are equal, however, it will place the contents of the Maps on the stack for further
   * comparisons.
   */
  private static boolean compareSortedMap(SortedMap map1, SortedMap map2, Deque stack,
      Set visited) {
    // Same instance check already performed...
   ...
   * @return true, if the passed in Class has a .equals() method somewhere between itself and just
   * below Object in it's inheritance.
   */
   ...
   *
   * @param c Class to check.
   * @return true, if the passed in Class has a .hashCode() method somewhere between itself and just
   * below Object in it's inheritance.
   */
