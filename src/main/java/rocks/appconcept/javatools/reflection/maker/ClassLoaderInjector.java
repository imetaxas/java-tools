...
public class ClassLoaderInjector {
  ...
  static {
    protectionDomain = (ProtectionDomain) AccessController
        .doPrivileged((PrivilegedAction) MethodInterceptor.class::getProtectionDomain);
    AccessController.doPrivileged((PrivilegedAction) () -> {
      try {
        Class<?> loader = Class.forName("java.lang.ClassLoader");
        defineClass = loader
            .getDeclaredMethod("defineClass", String.class, byte[].class, Integer.TYPE,
  ...
  public static Class<?> injectClass(ClassLoader loader, String className, byte[] classFile)
      throws Exception {
    Class<?> aClass = (Class) defineClass
        .invoke(loader, className, classFile, 0, classFile.length, protectionDomain);
