...
public class BeanMaker {
    ...
    public static byte[] makeBean(String classname, String baseclass, Map<String, String> properties, Map<String, String> lists) {
        ClassFile classFile = new ClassFile(ClassConstants.ACC_PUBLIC | ClassConstants.ACC_FINAL | ClassConstants.ACC_SUPER, dotToSlash(classname),dotToSlash(baseclass));
        ...
        properties.forEach((property, propertyType) -> {
            ...
            classFile.addMethod(ClassConstants.ACC_PUBLIC, getterName(property), "()" + typeName(propertyType), (short) 2, (short) 2, (cp, out) -> {
                out.writeByte(ClassConstants.opc_aload_0);
                out.writeByte(ClassConstants.opc_getfield);
                out.writeShort(cp.getFieldRef(dotToSlash(classname), property, typeName(propertyType).replaceAll("<[^>]+>+", "")));
                out.writeByte(returnOpcode(typeName(propertyType)));
            });
            classFile.addMethod(ClassConstants.ACC_PUBLIC, setterName(property), "(" + typeName(propertyType) + ")V", (short) 4, (short) 4, (cp, out) -> {
                out.writeByte(ClassConstants.opc_aload_0);
        ...
        lists.forEach((property, listType) -> {
            classFile.addField(ClassConstants.ACC_PRIVATE, property, "Ljava/util/List<" + typeName(listType) + ">;");
            classFile.addMethod(ClassConstants.ACC_PUBLIC, getterName(property), "()Ljava/util/List<" + typeName(listType) + ">;", (short) 2, (short) 2, (cp, out) -> {
                out.writeByte(ClassConstants.opc_aload_0);
                out.writeByte(ClassConstants.opc_getfield);
                out.writeShort(cp.getFieldRef(dotToSlash(classname), property, typeName(listType).replaceAll("<[^>]+>+", "")));
                out.writeByte(ClassConstants.opc_areturn);
            });
        });

        // create constructor which initializes all lists
        classFile.addMethod(ClassConstants.ACC_PUBLIC, "<init>", "()V", (short) 4, (short) 2, (cp, out) -> {
            out.writeByte(ClassConstants.opc_aload_0);
