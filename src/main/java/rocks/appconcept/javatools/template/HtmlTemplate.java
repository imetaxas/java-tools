...
public class HtmlTemplate {
  ...
  public static final Pattern TAG_PATTERN = Pattern.compile(
      "\\$([a-zA-Z]+)|\\$\\{([a-zA-Z]+)\\}|#if\\(\\$([a-zA-Z]+)\\)|(#end)|#include\\(\"([^\"]+)\"\\)|(?:#\\*.*?\\*#)",
      Pattern.DOTALL);
  ...
  public HtmlTemplate(String html, Map<String, String> presets) {
    int lastEnd = 0;
    for (Iterator<MatchResult> iterator = Patterns.allMatches(TAG_PATTERN, html).iterator();
        iterator.hasNext(); ) {
      ...
      if (variable != null) {
        ...
        while (true) {
          try {
            ...
            throw new RuntimeException(
                "Unbalanced if/end in template (pos=" + conditionStart + ")");
          }
        }
        ...
        if (presets != null && presets.containsKey(condition)) {
          if (HtmlTemplate.isTruthy(presets.get(condition))) {
            writers.addAll(
                new HtmlTemplate(html.substring(conditionStart, conditionEnd), presets).writers);
          }
        } else {
          writers.add(new ConditionalWriter(condition,
              new HtmlTemplate(html.substring(conditionStart, conditionEnd), presets).writers));
        }
