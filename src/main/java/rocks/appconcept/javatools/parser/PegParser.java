...
import rocks.appconcept.javatools.parser.peg.AstBase;
import rocks.appconcept.javatools.parser.peg.AstCall;
import rocks.appconcept.javatools.parser.peg.AstCharacterClass;
import rocks.appconcept.javatools.parser.peg.AstCharacterClassifier;
import rocks.appconcept.javatools.parser.peg.AstDigits;
import rocks.appconcept.javatools.parser.peg.AstDot;
import rocks.appconcept.javatools.parser.peg.AstList;
import rocks.appconcept.javatools.parser.peg.AstLiteral;
import rocks.appconcept.javatools.parser.peg.AstNot;
import rocks.appconcept.javatools.parser.peg.AstOptional;
import rocks.appconcept.javatools.parser.peg.AstOptions;
import rocks.appconcept.javatools.parser.peg.AstQuotedString;
import rocks.appconcept.javatools.parser.peg.AstRepeatPlus;
import rocks.appconcept.javatools.parser.peg.AstRepeatStar;
import rocks.appconcept.javatools.parser.peg.AstSequence;
import rocks.appconcept.javatools.parser.peg.AstSingleCharacter;
import rocks.appconcept.javatools.parser.peg.AstWhitespace;
import rocks.appconcept.javatools.parser.peg.AstWrapAsConstant;
import rocks.appconcept.javatools.parser.peg.AstWrapAsList;
import rocks.appconcept.javatools.parser.peg.AstWrapAsNewObject;
import rocks.appconcept.javatools.parser.peg.AstWrapAsSetProperty;
import rocks.appconcept.javatools.parser.peg.AstWrapAsString;
import rocks.appconcept.javatools.parser.peg.AstWrapPick;
...
public abstract class PegParser {
  ...
  /**
   * Parse the specified input. This method can return {@link #FAILED} if the input couldn't be
   * parsed. The {@link Input#position} field will be updated to reflect how much of the input was
   * consumed by the parser. A typical caller would ensure that the result is not {@link #FAILED}
   * and that {@link Input#position} is equal to the length of {@link Input#input}.
   *
   * @param input The input to parse.
   * @return The result of parsing the object. Will return {@link #FAILED} if the input couldn't be
   * parsed.
   */
  ...
  public static PegParser character(String characters) {
    return (characters.length() == 1) ? new AstSingleCharacter(characters.charAt(0))
        : new AstCharacterClass(characters);
  }
  ...
  public static PegParser literal(String literal) {
    return (literal.length() == 1) ? new AstSingleCharacter(literal.charAt(0))
        : new AstLiteral(literal);
  }

  public static PegParser literals(Collection<String> literals) {
    PegParser[] parsers = literals.stream().collect(Collectors.groupingBy(String::length))
        .entrySet().stream().sorted(Comparator.comparing(e -> -e.getKey()))
        .flatMap(r -> r.getValue().stream()).map(PegParser::literal).toArray(PegParser[]::new);
