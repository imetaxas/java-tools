...
public final class Profiler {
    ...
    public static String sampling(final long repetitions, final ProfileBlock runnable) {
        ...
        try {
            ...
            out.append("--------------------------------------------------\n");
            out.append("Profile based on ").append(totalSamples).append(" samples, ").append(totalTime).append("ms.\n");
            for (Iterator<Map.Entry<String, Integer>> iterator = samples.iterator(); iterator.hasNext(); ) {
                ...
                if (sample.getValue() == 1 || percentage == 0) {
                    ...
                    out.append("Rest of samples ~ ").append(totalSkippedPercentage).append("% (").append(totalSkippedSamples).append(" samples in total)\n");
                    break;
                }
                out.append(sample.getKey()).append(": ").append(percentage).append("% (").append(sample.getValue()).append(" samples)\n");
            }
            out.append("--------------------------------------------------\n");
            return out.toString();
